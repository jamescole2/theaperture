...
const navMenu=document.querySelector(".nav-menu"),navLinks=navMenu?Array.from(navMenu.querySelectorAll(".nav-link")):[],getIndicatorInset=()=>{if(!navMenu)return 0;const e=getComputedStyle(navMenu).getPropertyValue("--nav-indicator-inset");return parseFloat(e)||0};let activeNavLink=navLinks.find(e=>e.classList.contains("active"))||navLinks[0]||null;const updateNavIndicator=e=>{if(!navMenu||!e)return;const t=e.getBoundingClientRect(),a=navMenu.getBoundingClientRect(),n=getIndicatorInset(),o=t.left-a.left-n;navMenu.style.setProperty("--nav-indicator-width",`${t.width}px`),navMenu.style.setProperty("--nav-indicator-offset",`${o}px`)};const setActiveNavLink=e=>{e&&e!==activeNavLink&&(activeNavLink&&activeNavLink.classList.remove("active"),e.classList.add("active"),activeNavLink=e,updateNavIndicator(e))};window.addEventListener("load",(()=>updateNavIndicator(activeNavLink))),window.addEventListener("resize",(()=>updateNavIndicator(activeNavLink))),navLinks.forEach(e=>{e.addEventListener("click",(()=>setActiveNavLink(e)))});const observableSections=document.querySelectorAll("section[id]");if("IntersectionObserver"in window&&navLinks.length){const e=new IntersectionObserver((e=>{e.forEach((e=>{if(e.isIntersecting){const t=e.target.getAttribute("id"),a=navLinks.find((e=>e.getAttribute("href")==`#${t}`));a&&setActiveNavLink(a)}}))}),{threshold:.45});observableSections.forEach((t=>e.observe(t)))}
